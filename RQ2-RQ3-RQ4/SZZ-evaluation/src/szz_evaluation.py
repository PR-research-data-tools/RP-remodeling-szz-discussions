...
model_names = ["B-SZZ", "AG-SZZ", "L-SZZ", "R-SZZ", "MA-SZZ"]
...
def get_correct(input_file: str, output_file: str, model_name: str) -> MyResults:
    ...
    out_path = output_file.split(path.sep)
    filename = ''
    for folder in out_path[:-1]:
        filename += folder + path.sep
                ...
                if model_name not in java_filter or (model_name in java_filter and 'java' in langs):
                    ...
                    for bic in pred_bics:
                        ...
                        if bic in oracle_bics:
                            ...
                            out_file.write("{},{},{},{}\n".format(obj['id'], obj['repo_name'], obj['fix_commit_hash'], bic))
...
if __name__ == "__main__":
    ...
    for ds in dataset_versions:
        ...
        for szz in szz_versions:
            models_to_test.append(f'{szz}_{ds}_composed.json')

        for i in range(len(models_to_test)):
            out_wrongly_file = path.join('out', 'wrong', ds + '_' + model_names[i] + '-wrongly_identified.csv')
            folders = models_to_test[i].split('_')
            res_path = path.join('out', f"{folders[0]}", f'{ds}', f'sample_pr_{folders[0]}_{ds}.json')
            results_per_model.append(get_correct(res_path, out_wrongly_file, model_names[i]))
        ...
        with open('out/' + f'{ds}_recall-precision.csv', 'w') as out_file:
        ...
        with open('out/' + f'{ds}_overlap_vi_vj.csv', 'w') as out_file:
            ...
            # plt.show()
            plt.savefig('out/' + f'{ds}_heatmap.pdf', dpi=300, bbox_inches='tight')
            plt.close()

        # Print overlap vi but others
        with open('out/' + f'{ds}_overlap_vi_but_others.csv', 'w') as out_file:
        ...
        with open('out/' + f'{ds}_not-identified.csv', 'w') as out_file:
